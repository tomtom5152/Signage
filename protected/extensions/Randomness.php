...
class Randomness
{
    ...
    public static function randomBytes($length = 8, $cryptoStrong = true, $http = false)
    {
        ...
        if ($cryptoStrong) {
            ...
            // Try /dev/urandom directly. On Linux it may block so deal with that.
            if (false !== ($f = @fopen('/dev/urandom', 'r'))
            ...
            if (self::strlen($s) < $length
                ...
                && false !== ($r = @file_get_contents(
                    'https://www.random.org/cgi-bin/randbyte?format=f&nbytes=' . $length
                ))
